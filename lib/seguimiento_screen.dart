...
class TareaStore {
  ...
  Future<void> save(String userId) async {
    ...
    await prefs.setString(
      'tareas_$userId',
      json.encode(
        _tareas.map((e) => e.toJson()).toList(),
      ),
    );
  }

  Future<void> add(Tarea t, String userId) async {
    _tareas.add(t);

    await save(userId);
  }

  Future<void> update(Tarea t, String userId) async {
    await save(userId);
  }
}
...
class _SeguimientoScreenState
  ...
  Future<void> _cargarDatos() async {
    await TareaStore.instance.load(_userId);
  ...
  List<Tarea> get _tareas =>
      TareaStore.instance.tareas;
  ...
  Map<String, int> get _categoriasCompletadas {
    ...
    for (final t
        in _tareas.where((t) => t.completada)) {
      map[t.categoria] =
          (map[t.categoria] ?? 0) + 1;
    }
  ...
  Future<void> _agregarTarea() async {
    ...
    setState(() {
      TareaStore.instance._tareas.add(tarea);
  ...
  void _toggleCompletada(Tarea t) async {
    ...
    await TareaStore.instance.update(
      t,
      _userId,
    );
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  Text(
                    '${(_progresoSemana * 100 * _anim.value).toInt()}%',
                    style:
                        const TextStyle(
                      color: Colors.white,
                      fontSize: 24,
                  ...
                  Text(
                    '${_semana.where((t) => t.completada).length}/${_semana.length}',
                    style:
                        const TextStyle(
                      color:
                          Colors.white70,
